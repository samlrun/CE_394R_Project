...
```{r pedSeverityCorr, fig.align='center'}
# Pedestrian severe crash correlations ######################################

seg <- read_csv("data/CAMS_Crashes_14-21_extra.csv")
seg <- corrPed(seg,SEG_ID)
int <- read_csv("data/ISAM_Crashes_14-21_extra.csv")
int <- corrPed(int,INT_ID)

df <- data.frame(
  seg["Severities 3-5 (Injury)"],
  int["Severities 3-5 (Injury)"]) %>%
  rownames_to_column(var = "ped") %>%
  rename(
    Segment = "Severities.3.5..Injury.",
    Intersection = "Severities.3.5..Injury..1") %>%
  pivot_longer(
    cols = c('Segment','Intersection'), 
    names_to = "Type",
    values_to = "corr",)
...
ggplot(
  ...
  geom_text(
    ...
    size=3.0)+
  labs(
    title="Correlation Between Pedestrian Variables and Injury Crashes", 
